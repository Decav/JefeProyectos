...
| **R8d** | Pendiente | Balance de números + assets (iconos 30) + QA a nivel 20 — **HU-R8d creada (2026-09-18)** | Las 6 specs se sienten distintas y el arco a 20 funciona |
...
| 2026-09-18 | **HU-ESTETICA-20 creada (dev)** — implementación del HUD de combate, referenciando el frame `HU-ESTETICA-19 — Combat HUD` del `.pen`; player frame, target frame, skillbar, consumable bar, micro menú, target arrow, boss bar y level up — sin tocar la lógica |
| 2026-09-18 | **HU-R8d creada** — balance de números + QA a nivel 20 (cierre de R8): metas por piso (TTK/TTD), power budget de ítems, familias de skills, diferenciación de 6 specs, pociones al 25% del máximo (precio 15, decisión PM); solo números en configs, fino final en R9 |
| 2026-09-18 | **HU-ESTETICA-21 creada (dev)** — rediseño de Mochila (B) y Equipo+Stats (C), referenciando los frames `HU-ESTETICA-10/11` del `.pen`; pilas/rarezas/tooltips con acciones y desglose bajo demanda con resaltado slot↔stat — sin tocar la lógica |
| 2026-09-18 | **HU-R6.11 creada** — sistema de orden de UI manual: implementa el botón "Ordenar UI" del menú (placeholder de R6.10) con modo edición y cuadrícula de centro/simetría, frames del HUD configurables con persistencia por personaje (`profile.uiLayout`), y ventanas arrastrables por sesión sin guardar |
| 2026-09-18 | **Implementación cerrada según reporte:** R6.11 (ordenar UI), reskins EST-15/16/17/20/21 y sistema de UI implementados; **pendientes: HU-R8d (balance/QA)** y assets de enemigos (modelos EST-02 + animaciones, animador/diseñador) |
| 2026-09-18 | **HU-R8e creada + SKILLS_CATALOG v2.3** — Flecha venenosa (DoT single-target, sin stacks, reusa ticks de R6.9) para Puntería: **reemplaza "Flecha pesada"** como skill de nivel 1 (árboles intactos, 30 skills total), VFX verde veneno e icono; números bajo bandas de R8d |
| 2026-09-18 | **HU-PUBLICAR-01/02 creadas** — publicación de Vandrheim con la dungeon como place aparte: 01 (PO) crear/publicar `Dungeon_Helada`, accesos (amigos + secure within universe), Place ID y pruebas desde el cliente; 02 (dev) bootstrap del place, portal cross-place con servidor reservado (TeleportAsync + ShouldReserveServer), errores con feedback y validación de entrada |
| 2026-09-18 | **PUBLICAR-01 avanzando** — place **`Dungeon_Helada` publicado** según reporte del PO; **Place ID `125210265978348`** entregado al dev (config de PUBLICAR-02); pendiente: *Secure within universe only* y pruebas desde el cliente |
| 2026-09-22 | **Renumeración R6.x:** nameplates → **R6.12**; AoE pre-cast → **R6.13** (se libera el choque de numeración con R6.11 Ordenar UI) |
| 2026-09-22 | **R9 en HUs:** **R9a Party** (2–4 amigos, dungeon compartida, threat, loot personal — UI en EST-22), **R9b Polish** (SFX por categoría, screen shake sutil, Volumen/Gráficos reales del menú), **R9c Balance fino** (post-playtest, tolerancias ±10–15%, QA final demo estable) + **HU-ESTETICA-22** (diseño de party frames) |
| 2026-09-22 | **EST-22 entregada (diseño)** — frame `HU-ESTETICA-22 — Party Frames and Windows` en el `.pen`; referenciado en HU-R9a para el dev; verificación PM pendiente de MCP de Pencil |
| 2026-09-22 | **PUBLICAR-02 QA OK** — teleport cross-place publicado (pueblo → `Dungeon_Helada` servidor reservado → vuelta) verificado según reporte del PO; flujo de run compartida habilitado para R9a |
| 2026-09-22 | **Bug reportado:** Flecha venenosa desaparece al entrar a la dungeon (place con configs desactualizadas — el perfil viaja por DataStore pero la dungeon no tiene la skill/DoT). **HU-PUBLICAR-03 creada:** sync inicial hub ↔ Dungeon_Helada + regla permanente "Sync places" en el DoD de los RC futuros; Rojo como solución estructural opcional |
| 2026-09-22 | **PUBLICAR-03 cambio de estrategia:** sincronización = **copia completa del place** (publicar el hub como Dungeon_Helada) + flag `placeType` para el arranque — sin sync manual por módulo; checklist solo de verificación |
| 2026-09-22 | **HU-PUBLICAR-04 creada** — unificación de places: `DungeonConfig` con place IDs y `placeType` **derivado de `game.PlaceId`** (sin flag manual), arranque ramificado hub/dungeon, y sincronización por copia completa — el dev deja de editar módulos manualmente en `Dungeon_Helada` |
| 2026-09-22 | **PUBLICAR-04 ajustada (revisión dev):** la dungeon es una **compilación derivada que excluye `Workspace.Hub`** (nada de copia idéntica con el pueblo); UI del hub gateada por contexto; backup + verificación de Place ID antes de sobrescribir; `DungeonConfig` ya tiene los IDs (faltan helpers); docs del place a actualizar |
| 2026-09-22 | **PUBLICAR-04 decisiones PM cerradas:** publicación manual acotada aceptada para MVP (Open Cloud futuro opcional); exclusión solo `Workspace.Hub`; `hubPlaceId = 84186294312317`; backup explícito solo en la 1.ª publicación, releases por historial de versiones |
| 2026-09-22 | **Bugs reportados (PO):** (1) móvil no puede interactuar con NPCs/portales/diálogos (pide tecla E inexistente en touch); (2) móvil: micromenú vertical, player frame y target frame demasiado grandes (tapan la pantalla); (3) dungeon: al vencer un jefe de piso, el jugador spawnea en la entrada y se cae (la dungeon carga después que el jugador); enemigos caen al vacío → piso incompletable; (4) el miniboss troll no se targetea con click (solo TAB) |
| 2026-09-22 | **HU-R6.14 creada (batch 1 de bugs, unifica 4 HUs individuales):** interacción móvil (E/touch), UI móvil compacta, spawn de dungeon (gate de piso listo) y targeting del trol — un solo RC |
| 2026-09-22 | **HU-ESTETICA-26 creada** — pueblo completo de Vandrheim (diseñador): 5 casas sin interior, 3 con interior para los NPCs comerciales, murallas con portón, plaza central y casa comunal cerrada; generación procedural con entrega estática; **absorbe a HU-ESTETICA-03 (obsoleta)** |
